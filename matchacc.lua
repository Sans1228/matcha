...
for _,v in getgc() do
    if type(v) == "function" and islclosure(v) then
        cs = debug.getconstants(v)
        for _,c in cs do
            if type(c) == "string" and (c:match("0x%x+") or c == "Tamper Protection" or c == "TableCheck") then
...
for _,o in getgc(true) do
    if type(o) == "table" then
        dt = rawget(o, "Detected")
        if dt and type(dt) == "function" then
            hookfunction(dt, function() return true end)
        end
        kf = rawget(o, "Kill")
        if kf and type(kf) == "function" then
            hookfunction(kf, function() end)
        end
        dc = rawget(o, "Disconnect")
        if dc and type(dc) == "function" then
            hookfunction(dc, function() end)
        end
    end
end
...
PlayersGroup:AddButton('Teleport to Target', function()
    if not matchacc.AutoKill.Target or matchacc.AutoKill.Target == "" then
        Library:Notify("Matcha.cc - No target selected!", 3)
        return
    end
    ...
    Callback = function(Value)
        ...
        if Value then
            if not matchacc.AutoKill.Target then
                Library:Notify("Matcha.cc - please select a target before using 'Spectate'.", 3)
...
local function updateCamlock()
    ...
    local velocity = target.Character:FindFirstChild(part).AssemblyLinearVelocity
			    ...
			    if Target.Character:FindFirstChild("Humanoid") then
			        ...
			        if prev ~= nil and curr < prev then
			            ...
			            if matchacc.HitEffects.HitNotifications then
			                Library:Notify("Matcha.cc - AutoKill Hit: " .. Target.Name .. " - Health: " .. curr, matchacc.HitEffects.HitNotificationsTime)
			            end
...
RunService.Heartbeat:Connect(function(dt)
        ...
        if matchacc.AutoKill.Target ~= nil then
            ...
            Library:Notify("Matcha.cc - Target not found(Hide void).", 3)
            if matchacc.AutoKill.AutoKillDesync then
                RunService:BindToRenderStep("RestoreCFrame", 199, function()
                    localPlayer.Character.HumanoidRootPart.CFrame = SavedPosition
                    RunService:UnbindFromRenderStep("RestoreCFrame")
